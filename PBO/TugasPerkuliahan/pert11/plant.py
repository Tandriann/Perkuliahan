...
window = tk.Tk()
window.title("Tugas PBO - Tandrian - 20210801208")
...
class Plant:
    ...
    def tampilImage(self):
        self.src_img = Image.open(
            f"img/{self.getImage}").resize((50, 50), Image.ANTIALIAS)
...
def pilihBunga(jenis, *args):
    ...
    btn_beriAir = ttk.Button(frm, text="1.Beri Air", command=lambda: [
                             aksi(dataShow, label_tunas, jenis, btn_beriAir, info)])
    btn_beriAir.pack(padx=10, pady=5, fil="x", expand=True)

    btn_beriPupuk = ttk.Button(frm, text="2.Beri Pupuk", command=lambda: aksi(
        dataShow, label_tunas, jenis, btn_beriPupuk, info))
    btn_beriPupuk.pack(padx=10, pady=5, fil="x", expand=True)

    btn_exit2 = ttk.Button(frm, text="3.Exit", command=lambda: [main(
        label_tunas, judul2, btn_beriAir, btn_beriPupuk, btn_exit2, info, dataShow)])
...
def main(*args):
    ...
    btn_anggrek = ttk.Button(frm, text="1.Anggrek", command=lambda: [pilihBunga(
        bunga_anggrek, judul, btn_anggrek, btn_mawar, btn_melati, btn_exit)])
    btn_anggrek.pack(padx=10, pady=5, fil="x", expand=True)

    btn_mawar = ttk.Button(frm, text="2.Mawar", command=lambda: [pilihBunga(
        bunga_mawar, judul, btn_anggrek, btn_mawar, btn_melati, btn_exit)])
    btn_mawar.pack(padx=10, pady=5, fil="x", expand=True)

    btn_melati = ttk.Button(frm, text="3.Melati", command=lambda: [pilihBunga(
        bunga_melati, judul, btn_anggrek, btn_mawar, btn_melati, btn_exit)])
